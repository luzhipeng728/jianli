<template>
  <div>
    ...
    <div v-else>
      <!-- Select All Header -->
      <div v-if="selectable" class="flex items-center gap-3 mb-3 pb-3 border-b border-secondary-100">
        <el-checkbox
        ...
        <span class="text-sm text-secondary-500">
          {{ (selectedIds?.length || 0) > 0 ? `已选 ${selectedIds?.length} 项` : '' }}
        </span>
      </div>
      ...
      <div class="space-y-3">
        <div
          ...
          :class="[
            ...
            selectedIds?.includes(resume.id) ? 'ring-2 ring-primary-500 bg-primary-50/30' : ''
          ]"
        >
          <!-- Checkbox -->
          <div v-if="selectable" class="flex-shrink-0">
            <el-checkbox
              :model-value="selectedIds?.includes(resume.id) || false"
              @change="(val: boolean) => handleSelect(resume.id, val)"
              @click.stop
            />
          </div>
          ...
          <!-- Education Warnings - 学历风险（重点） -->
          <div class="flex-shrink-0 w-16 text-center">
            <span
              v-if="resume.education_warnings?.length"
              class="inline-flex items-center justify-center px-2 py-1 rounded-full bg-red-100 text-red-700 text-xs font-bold animate-pulse"
              title="学历造假风险"
            >
              学历风险
            </span>
            <span
              v-else-if="resume.warnings?.length"
              class="inline-flex items-center justify-center w-6 h-6 rounded-full bg-yellow-50 text-yellow-600 text-xs font-medium"
            >
...
const emit = defineEmits(['view', 'delete', 'select', 'select-all'])

const isAllSelected = computed(() => {
  return props.resumes.length > 0 && props.selectedIds?.length === props.resumes.length
})

const isIndeterminate = computed(() => {
  const len = props.selectedIds?.length || 0
  return len > 0 && len < props.resumes.length
})
